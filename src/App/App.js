...
function App() {
  ...
  if (displaySuccess[1] === "Request successfully") {
    setTimeout(() => {
      toast.success(displaySuccess[1], {
        type: "info",
        theme: "colored",
        autoClose: 5000,
      });
  ...
  useEffect(() => {
    async function fetchAllCharacters() {
      await axios
        .get(requests.fetchAllCharactersDatabase)
        .then((response) => {
  ...
  const handleForm = (e, searchValue) => {
    // console.log(
    //   "🚀 ~ file: App.js:188 ~ handleForm ~ searchValue:",
    //   searchValue,
    // );
    ...
    setCloseSearch(true);
    axios
      .get(requests.fetchMovieBySearch + searchValue)

      .then((response) => {
        const data = response.data.results;
        // console.log("fetchMovieBySearch: ", data[0]);
  ...
  return (
    <div className="App">
      ...
      <Router>
        ...
        {closeSearch ? (
          <div
            className={`${toggleSearch ? "card_movie_container_search " : "title-transparent"}`}
          >
            <button className="close-video" onClick={handleClickSearchClose}>
              x
            </button>
            <h1 className={`${toggleSearch ? "title" : "title-transparent"}`}>
              Résutat de la recherche pour : "{search}"
            </h1>
            {/* affichage du résulat de la barre de recherche */}
            {data &&
              data.map((movie, index) => (
                <div
                  className={`${toggleSearch ? "movie_container_search" : "movie_container_search-transparent"}`}
                  key={index}
                >
                  ...
                  <div className="movie_acceuil">
                    <img
                      ...
                      alt="image_film_tintin"
                      src={process.env.PUBLIC_URL + "/tintin/" + movie.picture}
                    />
                  </div>
                  <p className="movies__synopsis_search">{movie.synopsis}</p>
                  <button
                    type="button"
                  ...
                  >
                    <PlayCircleIcon />
                    <HashLink className="" smooth to="#movie-tintin">
                      Lecture
                    </HashLink>
                  </button>
                </div>
        ...
        <Routes>
          ...
          <Route
            path="/basket"
            element={<Basket todos={todos} data={character} />}
          />
          ...
          <Route
            path={`/editmovie/:id`}
            element={<EditMovie props={{ datas: dataMovie }} />}
          />

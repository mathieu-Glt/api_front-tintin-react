...
function TopRated() {
  ...
  async function fetchDatabase() {
    try {
      ...
      if (request.data.results) {
        setMovieRatedDatabase(request.data.results[0]);
      } else {
  ...
  return (
    <div>
      <h1 className="title_film_top_rated">
        LES MEILLEURS FILMS QUE J'AI SELECTIONNE
      </h1>
      <div>
        <div
          className={`${displayGuestMessage === 1 ? "displayMessage" : "transDisplayMessage"}`}
        >
          <img
            alt="image_tournesol"
            src={process.env.PUBLIC_URL + "/assets/Tournesol.jpg"}
          />
          <p>
            {" "}
            Je ne comprends pas mon pendule m'indique que mes films sont içi
            ?{" "}
          </p>
        </div>
        <div
          className={`${displayGuestMessage === 2 ? "displayMessage" : "transDisplayMessage"}`}
        >
          <img
            alt="haddock"
            src={process.env.PUBLIC_URL + "/assets/haddock.png"}
          />
          <p>
            {" "}
            Mille millions de mille sabords vous étes sourds ! Combien de fois
            faut il vous le répeter ! Vous devez être connécté !!!
          </p>
        </div>
      </div>
          ...
          <div key={index} className="movie_container_favories">
            ...
            <div className="image">
              <div className="movie_top_rated">
                <img
                  className="image_database_top_rated"
            ...
            <div className="movies__buttons ">
              ...
              {admin ? (
                <button className="banner_button">Supprimer</button>
              ) : null}
            </div>
          </div>
        ))}

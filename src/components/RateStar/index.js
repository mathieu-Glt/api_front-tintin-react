...
function RateStar(props) {
  ...
  function handleClick(value) {
    ...
    if (!userData) {
      return toast.success("Vous devez vous connecter ou vous enregistrer", {
        type: "error",
        theme: "colored",
        autoClose: 5000,
      });
      ...
      addRateMovie(value);
      setTimeout(() => {
        toast.success("Thank you Your rate is registered", {
          type: "info",
          theme: "colored",
          autoClose: 5000,
        });
    ...
    async function addRateMovie(value) {
      ...
      try {
        axios
          .patch(requests.fetchMovieRate, datas, {
            headers: {
        ...
        {stars.map((_, index) => {
          return (
            <FaStar
              ...
              color={
                (hoverValue || currentValue || props.movie.rating) > index
                  ? colors.orange
                  : colors.grey
              }
